...
tests <- tidyr::pivot_longer(
  ...
  dplyr::mutate(
    prev = round((tp + fp) / (tp + fp + tn + fn),3),
    se = round((tp) / (tp + fp), 3),
    sp = round((tn) / (tn + fn), 3)
  )
...
ui <- navbarPage(
  ...
  tabPanel(
    ...
    p("Translating diagnostic accuracy studies into a physio's practice can be a difficult task. By design, many diagnostic studies report mainly the Sensitivity (Sn) and Specificity (Sp) of the evaluated diagnostic tests. 
    ...
    p("This tab focuses on the translation of test combinations in your practice. It allows you to select multiple test that you want to perform in practice,
      ...
      mainPanel(
        tabsetPanel(
          ...
          and provides an overview of the pooled prevalence of the disease of interest across all studies. In total, ",
                   ...
                   p("The following graph depicts the number of TP, FN, TN, and FP tests in a patient population of 100 patients based
                         on an occurance that is identical to the pooled prevalence."),
                   HTML("<h4><i class='fa fa-user-injured' style='color:#D35400'></i>&nbsp; Patients with the disorder that you can identify</h4>"),
                   HTML("<h4><i class='fa fa-user-injured' style='color:#F89101'></i>&nbsp; Patients with the disorder that you will not identify</h4>"),
                   HTML("<h4><i class='fa fa-user' style='color:#3CAB48'></i>&nbsp; Patients without the disorder that you can identify</h4>"),
                   HTML("<h4><i class='fa fa-user' style='color:#FFCD01'></i>&nbsp; Patients without the disorder that you will wrongfully will classify</h4>"),
                   fluidRow(
                     column(
          ...
          tabPanel("Quality",
                   ...
                   p("The following chart provides information on the overall study quality. The original scores were recalculated as
          ...
          tabPanel("Patient",
                   ...
                   p("Based on your given pre-test probability, the probability that your patients effectively has the disorder is calculated based on your specific given
                   ...
                   fluidRow(
                     ...
                     p("Change the pre-test probability in the left side-bar to evaluate the probability that your patient has the disorder.
  ...
  tabPanel(
    ...
    sidebarLayout(
      ...
      mainPanel(
        ...
        p("In the list below, you can select the tests you have performed on your patient. It provides you with information on the
        ...
        # fluidRow(
        # prettyCheckboxGroup(
        #   inputId = "test_positives",
        #   label = "Select the tests that are positive:",
        #   choices = c("None"),
        #   inline = TRUE,
        #   status = "primary",
        ...
        fluidRow(
          ...
          h3("My patients tests positive on all tests:"),
          column(
            h3("NPV"),
...
server <- function(input, output, session) {
  ...
  quality <- reactive({
    qual_dat <- data() %>%
      mutate(Score = round(score / maxscore, 3)*100,
             Study = paste0(study, " (", year, ")"))
    
    hchart(qual_dat, "bullet", hcaes(x = Study, y = Score, target = 100), color = "black") %>%
      ...
      hc_yAxis(
        ...
        gridLineWidth = 0,
        plotBands = list(
          list(from = 0, to = 50, color = "#e74c3c"),
          list(from = 50, to = 80, color = "#f39c12"),
          list(from = 80, to = 100, color = "#00a65a")
        )
      ) %>%
      hc_xAxis(
        gridLineWidth = 15,
